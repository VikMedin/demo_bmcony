...
const profileOldBlockRegex = /\{adminTab === 'perfil' && \([\s\S]*?\/\* END OF ADMIN PANEL WORKSPACE CONTENT \*\//g;
...
const newProfileBlock = `{adminTab === 'perfil' && profile && (
              ...
              {/* END OF ADMIN PANEL WORKSPACE CONTENT */}`;

code = code.replace(profileOldBlockRegex, newProfileBlock);
