...
const profileOldBlockStart = "{adminTab === 'perfil' && (";
const indexOfProfile = code.indexOf(profileOldBlockStart);

if (indexOfProfile !== -1) {
  // Find the end of this block
  const endMarker = '</div>\n                    )}';
  let indexOfEnd = code.indexOf(endMarker, indexOfProfile);
  
  if (indexOfEnd !== -1) {
    // Delete the old profile block entirely and replace it
    const oldBlock = code.substring(indexOfProfile, indexOfEnd + endMarker.length);
    const newProfileBlock = `{adminTab === 'perfil' && profile && (
    ...
    code = code.replace(oldBlock, newProfileBlock);
  }
}
