...
// The messed up area is right after `1255 </div>` and before `{adminTab === 'perfil'`.
// Let's replace the whole tail from `auditLogs` to `</main>`.
// Actually, it's safer to just replace from `auditLogs.map` to the end of `</main>`.

const regex = /\{auditLogs\.map\(log => \([\s\S]*?<\/main>/;
